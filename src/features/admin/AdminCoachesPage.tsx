...
import { Mail, Phone, UserCog, Pencil } from 'lucide-react'
...
/**
 * Admin: Antrenörler.
 * Lists all coach profiles. Admin can view and deactivate/reactivate accounts.
 * Full edit form comes in a later phase.
 */
export function AdminCoachesPage() {
  ...
  const load = async () => {
    setIsLoading(true)
    const list = await listProfiles({ role: 'coach' })
    setCoaches(list)
    setIsLoading(false)
  }
